...
def get_scores(df, quiz_name=None, to_csv=True):
    """
    Input:
        df: a DataFrame generated by get_record
    """
    
    mask = (df.filename != 'ignore')
    scores = df.loc[mask, ['id','points']].groupby('id').mean().reset_index()
    
    if to_csv:
        scores.to_csv(quiz_name+'_scores.csv')
